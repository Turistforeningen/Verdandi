...
exports.saveUserData = data => {
  const { sherpa_id: _id, fornavn, etternavn, født, epost } = data;
        ...
        return exports.getUserData(token)
          .then(user => {
            if (user.sherpa_id !== id) {
              return Promise.reject(`User ${id} sent token belonging to ${user.sherpa_id}`);
            }
            return user;
          })
          .then(exports.saveUserData)
          .then(user => {
            return redis.set(`user:${id}:${token}`, JSON.stringify(user))
...
exports.middleware = (req, res, next) => {
  if (req.authClient && req.isAuthenticatedClient) {
    // Request is already authenticated as client
    return next();
  } else if (req.authUser && req.isAuthenticatedUser) {
    ...
    return exports.clientVerify(req.headers['x-client-token'])
      .then(client => {
        ...
        req.isAuthenticatedClient = true;
        next();
      })
...
exports.requireAuth = (req, res, next) => {
  if (req.authUser && req.isAuthenticatedUser) {
    return next();
  } else if (req.authClient && req.isAuthenticatedClient) {
    return next();
  }
...
exports.requireClient = (req, res, next) => {
  if (req.authClient && req.isAuthenticatedClient === true) {
    return next();
  }
  return next(new HttpError('X-Client-Token header is required', 401));
};

exports.requireUser = (req, res, next) => {
  if (req.authUser && req.isAuthenticatedUser === true) {
    return next();
  }
  return next(new HttpError('X-User-Id header is required', 401));
};

exports.isClient = req => !!(req.authClient && req.isAuthenticatedClient === true);

exports.isUser = (req, userId) => {
  if (req.isAuthenticatedUser !== true) {
    return false;
  }
  ...
  if (userId && (req.authUser._id !== userId)) {
    return false;
  }
